...
export function applyEvent(barbers:Barber[],event:SimEvent):Barber[]{
 return barbers.map(b=>{
  ...
  switch(event.type){
   case 'ASSIGN_WALKIN':return {...b,status:'WITH CLIENT',walkins:b.walkins+1,walkinAssignments:(b.walkinAssignments||0)+1,idle:0};
   case 'REJECT_WALKIN':return {...b,rejections:b.rejections+1};
   case 'CHECKOUT':return {...b,status:'AVAILABLE',revenue:b.revenue+event.amount,walkinRevenue:(b.walkinRevenue||0)+event.amount,completed:b.completed+1,clients:(b.clients||b.completed)+1,idle:0};
