...
const rows=value=>Array.isArray(value)?value:Array.isArray(value?.data)?value.data:Array.isArray(value?.items)?value.items:Array.isArray(value?.Results)?value.Results:[];
...
async function readDeltas(tenantId,locationId,since){
 ...
 const attempts=[
  ['/v1/dds/appointments',{StartDate:start}],
  ['/v1/dds/employees',{StartDate:start}],
  ['/v1/dds/sales',{StartDate:start}]
 ];
 const result={appointments:null,employees:null,sales:null};
 for(const [path,params] of attempts){
  const key=path.split('/').pop();
  try{result[key]=await meevoGet(path,{tenantId,locationId,params})}catch(error){result[key]={unavailable:true,message:error instanceof Error?error.message:'DDS unavailable'}}
 }
...
const server=http.createServer(async(req,res)=>{
 ...
 try{
  ...
  if(req.method==='POST'&&req.url==='/api/meevo/connect/start'){
   const body=await readBody(req);const tenantId=positiveInt(body.tenantId||body.companyCode);
   if(!tenantId)return json(res,400,{ok:false,error:'Enter a valid Meevo Tenant / Company ID.'});
   return json(res,200,{ok:true,organizationId:crypto.randomUUID(),tenantId:String(tenantId),requiresLocation:true,readOnly:true,message:'Tenant accepted. Verify one accessible location to initialize read-only synchronization.'});
  }

  if(req.method==='POST'&&req.url==='/api/meevo/connect/verify'){
   const body=await readBody(req);const tenantId=positiveInt(body.tenantId||body.companyCode),locationId=positiveInt(body.locationId);
   if(!tenantId||!locationId)return json(res,400,{ok:false,error:'Valid Tenant and Location IDs are required.'});
   const verified=await verifyMeevoLocation(tenantId,locationId);
   return json(res,200,{ok:true,readOnly:true,tenantId:String(tenantId),locationId:String(locationId),business:verified.business,locations:verified.locations});
  }

  if(req.method==='POST'&&req.url==='/api/meevo/bootstrap'){
   const body=await readBody(req);const tenantId=positiveInt(body.tenantId),locationId=positiveInt(body.locationId);
   if(!tenantId||!locationId)return json(res,400,{ok:false,error:'Valid Tenant and Location IDs are required.'});
   const data=await bootstrap(tenantId,locationId);
   return json(res,200,{ok:true,readOnly:true,syncedAt:new Date().toISOString(),tenantId:String(tenantId),locationId:String(locationId),...data});
  }
  ...
  if(req.method==='POST'&&req.url==='/api/meevo/employees'){
   ...
   if(!tenantId||!locationId)return json(res,400,{ok:false,error:'Valid Tenant and Location IDs are required.'});
   const employees=await meevoGet('/v1/employees',{tenantId,locationId,params:{PageNumber:0,ItemsPerPage:100,IsTerminated:false}});
   return json(res,200,{ok:true,employees,count:rows(employees).length});
  }

  if(req.method==='POST'&&req.url==='/api/meevo/sync'){
   const body=await readBody(req);const tenantId=positiveInt(body.tenantId),locationId=positiveInt(body.locationId);
   if(!tenantId||!locationId)return json(res,400,{ok:false,error:'Valid Tenant and Location IDs are required.'});
   const deltas=await readDeltas(tenantId,locationId,body.since);
   return json(res,200,{ok:true,readOnly:true,day:isoDay(),deltas});
  }
