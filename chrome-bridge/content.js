(() => {
  ...
  function compileBarbers(tokens){
    ...
    return anchors.map((a,i)=>{const c=center(a.percent),prev=i?center(anchors[i-1].percent):null,next=i<anchors.length-1?center(anchors[i+1].percent):null,left=prev==null?c-(next!=null?(next-c)/2:90):(prev+c)/2,right=next==null?c+(prev!=null?(c-prev)/2:90):(c+next)/2;const col=tokens.filter(t=>center(t)>=left&&center(t)<right&&t.y>hy+35),texts=col.map(t=>clean(t.text)).filter(Boolean),joined=texts.join(' | '),brk=/(^|\| )(?:LUNCH|BREAK)(?: \||$)/i.test(joined),busy=/(checked in|in progress|processing|with client)/i.test(joined);const appointmentYs=[];for(const t of col.filter(t=>/(signature cut|fade cut|hot lather|beard trim|haircut|shave|color|facial|permed|express facial|\bcut\b)/i.test(clean(t.text))).sort((a,b)=>a.y-b.y)){if(!appointmentYs.some(y=>Math.abs(y-t.y)<32))appointmentYs.push(t.y)}return{name:clean(a.name.text),occupancy:num(a.percent.text),appointments:appointmentYs.length,status:brk?'BREAK':busy?'WITH CLIENT':'AVAILABLE',column:{left,right},evidence:{headerY:hy,tokenCount:col.length}}})
  }
